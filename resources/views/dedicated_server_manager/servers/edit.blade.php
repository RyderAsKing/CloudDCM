    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Editing server ' . $server->hostname) }}
            </h2>
                    ...
                    <div x-show="modalOpen"
                        ...
                        <div x-show="modalOpen" x-trap.inert.noscroll="modalOpen"
                            ...
                            <div class="flex flex-col-reverse sm:flex-row sm:justify-between sm:space-x-2">
                                ...
                                <form action="{{route('dedicated_server_manager.servers.destroy', $server->id)}}"
                                    method="post">
                                    @method('DELETE')
                                    @csrf
                                    <button type="submit"
    ...
    <div>
        <div class=" mx-auto ">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                ...
                <form action="{{ route('dedicated_server_manager.servers.update', $server) }}" method="POST"
                    ...
                    <div class="flex flex-col mt-2">
                        <label for="server_size" class="font-bold">Location</label>
                        <select name="location_id" id="location_id">
                            <option value=" " @if($server->location_id == null) selected @endif>Uncategorized</option>
                            @foreach ($locations as $location)
                            <option value="{{ $location->id }}" @if($server->location_id == $location->id) selected
                                @endif>{{
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        ...
                        <div class="flex flex-col ">
                            ...
                            <select name="cpu" id="cpu"
                                ...
                                @for($i = 1; $i < 17; $i++) <option value="{{ $i }}" @if($server->cpu == $i) selected
                                    @endif>{{ $i }}</option>
                                    @endfor
                            </select>
                            @error('cpu')
                        ...
                        <div class="flex flex-col ">
                            ...
                            <x-text-input name="os" label="os" placeholder="eg Windows 11" value="{{$server->os}}" />
                            @error('os')
                            <x-input-error :messages="$message" />
                            @enderror
                        </div>
                    </div>
                    <x-primary-button class="mt-2" style="width: fit-content;">Edit server &rarr;</x-primary-button>
                </form>
            </div>
        </div>
    </div>
